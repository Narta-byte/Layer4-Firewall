...
class PolicyBuilder:
    ...
    def combine_2d_lists(self, list_2d):
        ...
        if sublist_len == 2:
            return [[list_2d[i][0], list_2d[j][1]] for i in range(len(list_2d)) for j in range(len(list_2d))]

        elif sublist_len == 3:
            return [[list_2d[i][0], list_2d[j][1], list_2d[k][2]] for i in range(len(list_2d)) for j in range(len(list_2d)) for k in range(len(list_2d))]
            ...
            if temp.__contains__("*") or any(self.contains_star_and_digit(item) for item in temp):
                ...
                for i, field in enumerate(rule):
                    ...
                    if self.contains_star_and_digit(field):
                        ...
                        if (self.contains_star_and_digit(new_rule[i])or new_rule[i] == "*"                        ):
                        ...
                        if new_rule[i] == "*":
                            logging.debug("new rule i == * "+ str(new_rule[i])+ " With "+ str(new_rule)+ " And rule: "+ str(rule))
                            # temp[i] = new_rule[i]
                ...
                if rule.count("*") > 1:
                    ...
                    for rest in self.store_inserted:  # rest_rules_elements:
                        ...
                        range_indices = [i for i, element in enumerate(temp) if "*" in element and any(char.isdigit() for char in element)]
                        ...
                        if missing_numbers:
                            ...
                            if (self.contains_star_and_digit(rule[missing_numbers[0]])and new_rule[missing_numbers[0]] != "*" and not self.contains_star_and_digit(new_rule[missing_numbers[0]])):
                        ...
                        logging.debug("WILDCARD INDICIES after if: " + str(wildcard_indices))
                        #wildcard_indices = [0,1,2]
