...
class PolicyTrieTree(trieTree.TrieTree):
    ...
    def insert(self, key, codeword):
        ...
        for level in range(length):
            ...
            if tempKey in child:
                ...
                if level == length - 1:
                    logging.debug(
                        f"Adding codeword to existing node key: {key} length: {length} codeword: {codeword} tempKey: {tempKey} level: {level}"
                    )
    ...
    def getCodeword(self, key):
        ...
        length = len(key)
        crawl = self.root
        best_match = crawl.codeword

        for level in range(length):
            child = crawl.children
            temp_key = key[level]
            if temp_key in child:
                crawl = child[temp_key]
                if crawl.codeword:
                    best_match = crawl.codeword
            else:
                return False, best_match

        return bool(crawl.codeword), crawl.codeword
